...
const firebaseConfig = {
  ...
  async function openProfileSelection(user, platform) {
      ...
      fetchPlatformStats(); // NEW: Fetch total for this platform
      showLoading(false);
  }
  ...
  function getProfileData() {
      ...
      return sortedKeys.map(k => {
          ...
          return {
              key: `Profile ${num}`, // Matches database "Profile 1"
              dbKey: k, // Matches config "profile1"
              name: config[k]
          };
      });
  }
  ...
  function renderProfileSelectionList() {
      ...
      profiles.forEach(p => {
          ...
          // Fix: Use p.key (Profile 1) instead of p.dbKey (profile1)
          card.onclick = () => openProfileFeed(p.key, p.name);
          
          // REMOVED: Quick refresh button HTML
          card.innerHTML = `
              <div>
  ...
  // REMOVED: handleQuickRefresh function
  ...
  function updateHeaderStats() {
      ...
      document.getElementById('stat-avg-views').innerText = formatViews(avgViews);
  ...
  async function submitNewVideo() {
      ...
      const newVideo = {
          ...
          status: "Pending", 
          views: 0,
          // We still save the date, but we don't sort by it in Firestore query
  ...
  function sortVideos(videos) {
      return videos.sort((a, b) => {
          if (currentSortOrder === 'views') {
              ...
              // Handle missing createdAt for old videos
